...
import { dateKey, goalAchieved, goalProgressFor, milestonesFor, personById } from "@/lib/utils";
  ...
  return (
    <AppShell>
      <header className="mb-6">
        <h1 className="font-display text-[28px] font-semibold leading-tight text-ink">Samen</h1>
        <p className="mt-1 text-sm text-muted">Dit telt net zo zwaar als de afspraken. Vaak zwaarder.</p>
      </header>
      ...
      {familyGoals.length > 0 ? (
        <section className="mt-8">
          <SectionTitle
            action={
              <Link href="/kind/doelen" className="text-sm text-primary hover:underline">
                Al onze doelen
              </Link>
            }
          >
      ...
      <section className="mt-8">
        <SectionTitle>Hoe we het thuis verdelen</SectionTitle>
        <ResponsibilityMap data={data} people={data.people} />
      </section>

      <section className="mt-8">
        <SectionTitle>Momenten samen</SectionTitle>
        {upcoming.length === 0 ? (
          <EmptyState
